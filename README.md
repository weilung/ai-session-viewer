...
> | | |
> |---|---|
> | 跑到哪一步 | 已跑到這套流程的**最高一層**（跨廠商、最強型號的獨立審查），共 5 道；另有 5 道同一家族模型的獨立審查（換脈絡，不等於跨廠商）|
...
> | 修正本身可靠嗎 | **也要打折。**後續每一道審查都會在**前一道的修正**上再找到東西：有些是修正新造的問題，有些是「修對了、但只涵蓋了一部分情況」（例如某個防呆只在最常見的路徑上生效）。後者不會讓任何測試變紅，所以**「這一格修過了」不等於「這一格沒問題了」**|
>
