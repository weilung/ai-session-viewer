...
前文內容變動／換模型／前文不在快取／快取暫時不可用，部分還附「被迫重算了多少 token」，
本工具據此標出成因並估算多付了多少錢。兩件事因此看得見了：
...
成因判定分兩級著色——**真失效**（閒置過期、1 小時內異常提早失效）才紅底白字；**結構性、本來就不會命中的**
（session 首呼叫全新前綴、回合內快取斷點的一次性 miss、換模型、壓縮後、切帳號）改中性灰並在滑鼠提示寫明
