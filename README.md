...
- **單一對話頁**：頁頂有「💲估算花費 · ⚡快取命中% · 脈絡峰值 · 產出」；每一則 assistant 還有可勾選的小徽章
  ...
  **距上一步 / ⏱耗時 / 寫入 TTL / effort**，預設顯示快取、失效原因與花費，其餘按需勾開，
  勾選狀態記在瀏覽器）。
  「新輸入 / 快取寫入 / 快取讀取」就是把單一 token 量攤成四種來源，想細看某輪吃了多少
  快取、寫了多少新快取時很有用；「脈絡」在知道該模型視窗時會附**佔視窗 %**（離自動壓縮還有多遠）。
- **寫入 TTL**：這一步把快取寫成 **5 分鐘**還是 **1 小時**。⚠ 它只描述**寫入**——同一步仍可能
  讀到先前用另一種 TTL 寫進去的前綴，所以徽章寫的是「寫入 TTL」，不是「這步的快取只剩 5 分鐘」。
  檔次從 1h 轉成 5m 的那一步會標成 `1h→5m`，session 表頭也會寫「寫入 TTL：1h N 步／5m M 步
  （HH:MM 起 1h→5m）」；**全程 1h（最常見）就完全不顯示**。實務上兩種情形會看到 5m：
  **子代理固定寫 5m**，以及**主對話中途切換**（本機實測對應帳號進入 usage overage——之後只要
  閒置超過 5 分鐘回來就是整段重寫）。表頭摘要只計主對話。
