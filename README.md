...
- **提早失效 vs 伺服器時段（實驗性）**：只取 1 小時快取「應命中帶」（閒置 2–55 分）的相鄰步驟，依
  **UTC（伺服器時間）**看各時段的提早失效率（dot plot＋信賴區間），並依間隔分層做 CMH 勝算比檢定
  「全球尖峰（13–21 UTC）是否較易失效」——先控制「午休/夜間本來就閒得久」的混雜，樣本不足會直說。
- **什麼時候撞到 limit**：附帶統計 429 撞牆時刻的分佈。

純估算、只統計 Claude 主對話（子代理／Codex 不納入）；②③時間用本機時區、④用 UTC。資料存在每個 session 的
